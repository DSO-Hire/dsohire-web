/**
 * BrandLockup — THE canonical DSO Hire lockup (single source for every surface).
 *
 * CANON (locked 2026-07-07): the wordmark is OUTLINED vector paths generated
 * from Manrope — DSO 26px/800, letter-spacing -0.8, natural width; HIRE
 * 9.5px/700, ink-justified flush to DSO (0px delta both edges, raster-verified).
 * NEVER re-typeset the wordmark as live <text> — live text is what caused the
 * logo drift (fallback fonts on export; see Brand Assets/logo-files-v2-d-form/
 * README.md). The SVG masters + PNG exports there were generated from these
 * SAME paths, so web, uploads, decks, and print are pixel-identical.
 *
 * Dark handling: `dark` forces the reversed treatment (ivory ink +
 * heritage-BRIGHT #8DB8A3 accents — flat heritage fails contrast on navy).
 * Without `dark`, token classes follow the app theme (ink/heritage flip in
 * .dark). Dependency-free on purpose — safe to import anywhere, client
 * wizards and the OG image included (satori needs the explicit `dark` fills).
 */
