    ...
    <form action={action} className="space-y-5 max-w-[480px]">
      ...
      {state.ok && state.message && (
        <div className="bg-success-bg border-l-4 border-heritage p-4">
          <p className="text-sm text-heritage-deep font-semibold">{state.message}</p>
        </div>
      )}
      ...
      <button
        type="submit"
        disabled={pending}
        className="inline-flex items-center gap-2.5 px-7 py-3 bg-primary text-primary-foreground text-xs font-bold tracking-[2px] uppercase hover:bg-primary/90 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
      >
