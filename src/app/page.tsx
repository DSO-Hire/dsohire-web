        ...
        <div className="grid grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_1fr_1fr] gap-10 lg:gap-14 pb-12 border-b border-white/10 mb-9">
          <div>
            ...
            <p className="text-[13px] text-ivory/55 leading-[1.7] mt-5 max-w-[280px]">
              The job board built for dental support organizations. One flat monthly fee.
              Unlimited multi-location postings. Made by operators, for operators.
            </p>
          </div>
...
function FooterLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <li>
      <Link href={href} className="text-[13px] text-ivory/60 hover:text-ivory transition-colors">
        {children}
      </Link>
    </li>
  );
...
/* ═══════════════════════════════════════════════════════
   SHARED — BRAND MARK + SECTION HELPERS
═══════════════════════════════════════════════════════ */

function BrandMark({ dark }: { dark?: boolean }) {
  const stroke = dark ? "currentColor" : "#14233F";
  return (
    <svg
      width="32"
      height="32"
